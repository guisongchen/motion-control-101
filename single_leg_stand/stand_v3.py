...
# ---------------------------------------------------------------------------
# Simplified phase config
# ---------------------------------------------------------------------------
PHASE_CONFIG = {
    "settle_time": 0.50,
    "shift_time": 8.00,
    "shift_roll_delta": 0.08,
    "min_contact_force": 80.0,
    "max_com_vel": 0.12,
    "max_L_norm": 0.80,
    "com_vel_ready_thresh": 0.50,
    "force_ratio_min": 0.30,
    "com_margin": 0.03,
    "slip_weight": 500000.0,
    "ds_posture_blend": 0.50,
    "ds_shift_posture_blend": 0.05,
    "adaptive_ratio_span": 0.35,
    "ds_force_target_margin": 0.02,
    "ds_force_ratio_cap": 0.90,
    "balance_entry_time": 0.30,
    "balance_hold_force": 15.0,
    "transition_blend_time": 0.20,
    "balance_max_tau_blend": 0.85,
    "swing_ramp_time": 1.2,
    "swing_progress_max": 0.7,
...
def compute_swing_progress(elapsed, phase, cfg):
    if phase != PhaseId.BALANCE:
        return 0.0
    swing_ramp = min(1.0, elapsed / max(cfg.get("swing_ramp_time", 1.5), 1e-6))
    swing_ramp = smoothstep(swing_ramp)
    max_progress = cfg.get("swing_progress_max", 0.45)
...
def compute_safe_tau(
    ...
    if phase >= PhaseId.SHIFT and swing_leg_dof_indices:
        ...
        if phase == PhaseId.SHIFT:
            progress = min(1.0, elapsed / max(cfg["shift_time"], 1e-6))
...
def compute_roll_delta(
    ...
    cfg = PHASE_CONFIG
    if phase_id < PhaseId.SHIFT:
        return 0.0
    if phase_id == PhaseId.SHIFT:
        elapsed = t - phase_start_time
        progress = smoothstep(elapsed / max(cfg["shift_time"], 1e-6))
        return progress * cfg["shift_roll_delta"]
    return cfg["shift_roll_delta"]  # BALANCE
...
def desired_support_ratio(phase, elapsed, cfg):
    if phase < PhaseId.SHIFT:
        return 0.5
    if phase == PhaseId.SHIFT:
        progress = np.clip(elapsed / max(cfg["shift_time"], 1e-6), 0.0, 1.0)
...
def compute_c_ref(phase, elapsed, phase_entry_com_xy, nominal_c_ref,
    ...
    if phase < PhaseId.SHIFT:
        ...
        if lipm_traj is not None and lipm_elapsed is not None:
            ...
            t_total = max(cfg["shift_time"], 1e-6)
...
def print_results(log: SimLog, robot: RobotModel, initial_foot_pos: dict):
    ...
    if max_phase >= PhaseId.SHIFT:
        sh_ratios = [log.support_ratio[i] for i, p in enumerate(log.phase) if p >= PhaseId.SHIFT]
        if sh_ratios:
            print(f"  SHIFT max ratio: {max(sh_ratios):.3f}, mean: {np.mean(sh_ratios):.3f}")
    if max_phase >= PhaseId.BALANCE:
        ss_indices = [i for i, p in enumerate(log.phase) if p == PhaseId.BALANCE]
        if ss_indices:
            ...
            print(f"  BALANCE duration: {ss_duration:.2f}s")
            print(f"  BALANCE CoM RMSE: {ss_rmse:.4f} m, max deviation: {ss_max_dev:.4f} m")

    reached_ss = max_phase >= PhaseId.BALANCE
    passed = reached_ss and rmse < RMSE_THRESH and max_slip < SLIP_PEAK_THRESH and net_slip < SLIP_NET_THRESH
    print(f"\nVerdict: {'PASS' if passed else 'FAIL'} "
          f"(reached BALANCE: {'yes' if reached_ss else 'no'}, "
...
def compute_ds_applied_tau(
    ...
    if step % wbc_period != 0 and last_ds_wbc_tau is not None:
        blend = cfg["ds_posture_blend"] if phase == PhaseId.SETTLE else cfg.get("ds_shift_posture_blend", 0.25)
...
class SingleSupportController:
    ...
    def compute(
        ...
        if not self.ss_established and ss_elapsed > cfg["balance_entry_time"]:
            support_metrics_now = self.robot.get_contact_metrics(support_leg_link)
            if support_metrics_now["normal_force"] >= cfg["balance_hold_force"]:
        ...
        transition_alpha = min(1.0, ss_elapsed / max(cfg["transition_blend_time"], cfg["balance_entry_time"], 1e-6))
        support_alpha = min(1.0, support_force_val / max(cfg["balance_hold_force"], 1e-6))
        effective_alpha = min(cfg["balance_max_tau_blend"], transition_alpha * support_alpha)
...
def main() -> None:
    ...
    phase = PhaseId.SETTLE
    phase_start_time = 0.0
    shift_gate = StabilityGate()
    c_ref = nominal_c_ref.copy()
    ...
    print(f"Phases: SETTLE -> SHIFT -> BALANCE")
    print("=" * 60)

    for step in range(total_steps):
        t = step * DT_SIM
        state = estimator.update(lock_support=(phase >= PhaseId.SHIFT))
        c = state["c"]
        c_dot = state["c_dot"]
        q = state["q"]
        v = state["v"]
        ...
        if phase == PhaseId.SETTLE:
            if elapsed >= cfg["settle_time"]:
                next_phase = PhaseId.SHIFT
            ...
            checks = {
                "time": elapsed >= cfg["shift_time"],
                "support_force": load_shift_metrics.support_force >= cfg["min_contact_force"],
                "support_ratio": load_shift_metrics.support_ratio >= cfg["force_ratio_min"],
                "com_speed": load_shift_metrics.com_speed <= cfg["com_vel_ready_thresh"],
            }
            if shift_gate.check(all(checks.values()), t, 0.10):
                next_phase = PhaseId.BALANCE
        ...
        if phase in (PhaseId.SETTLE, PhaseId.SHIFT):
    ...
    plot_diagnostics(
        ...
        ds_min_force=cfg["min_contact_force"],
        load_shift_roll_delta=cfg["shift_roll_delta"],
        ds_max_L_norm=cfg["max_L_norm"],
    )
